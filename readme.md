...
A stack is a collection of elements. It can be thought of as a physical stack of items. When an element is added (pushed) to a stack, it goes to the "top" of the stack. When an item is taken out of the stack, it is also taken (popped) from the "top" of the stack. This means that the elements which are added last, are removed first and the elements which are added first, are removed last.
